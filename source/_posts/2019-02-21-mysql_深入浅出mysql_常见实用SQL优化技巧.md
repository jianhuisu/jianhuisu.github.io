...
临时表类似于正常表,可以进行增删改查,但是临时表不会在`show tables`的列表中显示
...
- 临时表只存在于当前线程中,线程结束,临时表自动drop(这意味着可以在多个线程中使用相同的临时表名)
- 临时表的操作不会写入到binlog中,所以临时表不会同步到`mysql cluster`中的其它`node`
- 直接将查询结果导入临时表 `CREATE TEMPORARY TABLE tmp_table SELECT * FROM table_name`(这个特别方便 省去手动创建字段)
- 在同一个query语句中,只能使用一次临时表的表名
- 临时表只能用在 memory,myisam,merge,或者innodb引擎

临时表的应用场景

	多表关联的结果集需要在当前会话中反复使用时,使用temporary table可以减少对资源的占用,降低程序的复杂度

Tips:

中间表

	是业务逻辑中的抽象概念,将计算结果先保存在一个临时的表中，然后再从这个表中计算，减少程序的复杂度。临时表是生成中间表时多采用的一种技术。
...
内部临时表：
	...
	内部临时表是MySQL在优化阶段或者执行阶段自动创建并用来存储某些操作的中间结果表。这种内部表对用户来说是不可见的，但是通过`EXPLAIN`或者`SHOW STATUS`可以查看MYSQL是否使用了内部临时表。
	比如下文中`Extra`列中内容
...
表结构建在磁盘里，数据在内存里 ，当停止服务后，表中的数据丢失，表结构保留。

## 小结

在索引使用方面，语句越简单越好，这样**执行计划稳定**，一定要使用绑定变量，减少语句解析，尽量减少表关联，尽量减少分布式事务。
用户并发大，用户的请求十分密集时，批量更新要分批进行，避免堵塞。
